...
# Etiquetas de datos en R

COmo funcionan actualmente con haven por ejemplo.

Ventjas del enfoque actual:

1. Las etiquetas no funcionan con bases de datos.Como las etiquetas se asignan una a una es complicado. (Mostar un ejemplo con un sqlite en memoria)
2. Es fácil compartir el diccionario de datos.
...
```{r}
str(set_labels(enft, dict = dict, vars = c("SEXO")))
```
...
enft2 <- set_labels(enft2, dict = dict, vars = c("Genero"), ignore_case = T)
